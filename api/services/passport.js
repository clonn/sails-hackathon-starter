...
passport.connect = async function (req, query, profile, next) {
  ...
  if (!req.user) {
    ...
    if (!passport) {
      try {
        // create user and update passport
        let userItem = await User.create(user);
        query.UserId = userItem.id;

        query.tokens = query.tokens.accessToken || query.tokens;
        await Passport.create(query);
        return next(null, userItem);
      } catch (err) {
        console.error(err);
    ...
    // Action:   Get the user associated with the passport.
      ...
      passport.save(function (err, passport) {
        ...
        User.find(passport.user.id, next);
      });
    }
  } else {
...
passport.callback = function (req, res, next) {
  ...
  if (provider === 'local' && action !== undefined) {
    if (action === 'register' && !req.user) {
      ...
      let identifier = req.body.username || req.body.email || 0;
...
passport.disconnect = function (req, res, next) {
  ...
  Passport.find(query, function (err, passport) {
    if (err) {
      return next(err);
    }
